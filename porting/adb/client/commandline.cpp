...
int read_and_dump_protocol(char **output_buffer, size_t *output_size, borrowed_fd fd, StandardStreamsCallbackInterface* callback) {
    ...
    while (protocol->Read()) {
      if (protocol->id() == ShellProtocol::kIdStdout) {
        capture_printf(output_buffer, output_size, protocol->data());
...
int read_and_dump(char **output_buffer, size_t *output_size, borrowed_fd fd, bool use_shell_protocol,
                  StandardStreamsCallbackInterface* callback) {
    int exit_code = 0;
    if (fd < 0) return exit_code;

    if (use_shell_protocol) {
      exit_code = read_and_dump_protocol(output_buffer, output_size, fd, callback);
    } else {
...
static int RemoteShell(char **output_buffer, size_t *output_size, bool use_shell_protocol, const std::string& type_arg, char escape_char,
    ...
    int exit_code = read_and_dump(output_buffer, output_size, fd, use_shell_protocol);
...
static int adb_shell(char **output_buffer, size_t *output_size, int argc, const char** argv) {
    ...
    return RemoteShell(output_buffer, output_size, use_shell_protocol, shell_type_arg, escape_char, command.empty(),
                       service_string);
}
...
static int adb_abb(int argc, const char** argv) {
    ...
    return RemoteShell(nullptr, nullptr, use_shell_protocol, shell_type_arg, escape_char, empty_command,
                       service_string);
}
...
#endif
    return adb_shell(nullptr, nullptr, argc, argv);
}
...
//   the data transfer.
//
static int adb_sideload_install(char **output_buffer, size_t *output_size, const char* filename, bool rescue_mode) {
    // TODO: use a LinePrinter instead...
    struct stat sb;
    if (stat(filename, &sb) == -1) {
        capture_printf(output_buffer, output_size, "adb: failed to stat file %s: %s\n", filename, strerror(errno));
        fprintf(stderr, "adb: failed to stat file %s: %s\n", filename, strerror(errno));
        return -1;
    }
    unique_fd package_fd(adb_open(filename, O_RDONLY));
    if (package_fd == -1) {
        capture_printf(output_buffer, output_size, "adb: failed to open file %s: %s\n", filename, strerror(errno));
    ...
    if (device_fd < 0) {
        capture_printf(output_buffer, output_size, "adb: sideload connection failed: %s\n", error.c_str());
        ...
        capture_printf(output_buffer, output_size, "adb: trying pre-KitKat sideload method...\n");
    ...
    while (true) {
        if (!ReadFdExactly(device_fd, buf, 8)) {
            capture_printf(output_buffer, output_size, "adb: failed to read command: %s\n", strerror(errno));
        ...
        if (strcmp(kMinadbdServicesExitSuccess, buf) == 0 ||
            ...
            capture_printf(output_buffer, output_size, "\rTotal xfer: %.2fx%*s\n",
        ...
        if (offset >= static_cast<int64_t>(sb.st_size)) {
            capture_printf(output_buffer, output_size,
        ...
        if (adb_lseek(package_fd, offset, SEEK_SET) != offset) {
            capture_printf(output_buffer, output_size, "adb: failed to seek to block: %s\n", strerror(errno));
            fprintf(stderr, "adb: failed to seek to package block: %s\n", strerror(errno));
            return -1;
        }
        if (!ReadFdExactly(package_fd, buf, to_write)) {
            capture_printf(output_buffer, output_size, "adb: failed to read package block: %s\n", strerror(errno));
        ...
        if (!WriteFdExactly(device_fd, buf, to_write)) {
            ...
            capture_printf(output_buffer, output_size, "adb: failed to write data '%s'\n", error.c_str());
            fprintf(stderr, "adb: failed to write data '%s' *\n", error.c_str());
            return -1;
        }
        xfer += to_write;
        ...
        if (percent != last_percent) {
            capture_printf(output_buffer, output_size, "\rserving: '%s'  (~%d%%)    ", filename, percent);
...
int send_shell_command(const std::string& command, bool disable_shell_protocol,
    ...
    return read_and_dump(nullptr, nullptr, fd.get(), use_shell_protocol, callback);
}
...
static int adb_connect_command(const std::string& command, TransportId* transport,
    ...
    read_and_dump(nullptr, nullptr, fd, false, callback);
    return 0;
}
...
static int adb_query_command(char **output_buffer, size_t *output_size, const std::string& command) {
    std::string result;
    std::string error;
    ...
    // printf("%s\n", result.c_str());
    capture_printf(output_buffer, output_size, "%s\n", result.c_str());
    return 0;
}
...
int adb_commandline(char** out_buf, size_t* out_buf_size, int argc, const char** argv) {
    ...
    while (argc > 0) {
        if (!strcmp(argv[0], "server")) {
            ...
            if (argc < 2) {
                // error_exit("--reply-fd requires an argument");
                capture_printf(out_buf, out_buf_size, "--reply-fd requires an argument\n");
                return 1;
            }
            ...
            if (!_is_valid_ack_reply_fd(ack_reply_fd)) {
                ...
                capture_printf(out_buf, out_buf_size, "adb: invalid reply fd \"%s\"\n", reply_fd_str);
                return 1;
            }
        } else if (!strcmp(argv[0], "--one-device")) {
            if (argc < 2) {
                // error_exit("--one-device requires an argument");
                capture_printf(out_buf, out_buf_size, "--one-device requires an argument\n");
                return 1;
            }
            ...
            if (isdigit(argv[0][2])) {
                ...
                if (argc < 2 || argv[0][2] != '\0') {
                    ...
                    capture_printf(out_buf, out_buf_size, "-s requires an argument\n");
                    return 1;
                }
                serial = argv[1];
                --argc;
            ...
            if (isdigit(argv[0][2])) {
                ...
                if (argc < 2 || argv[0][2] != '\0') {
                    // error_exit("-t requires an argument");
                    capture_printf(out_buf, out_buf_size, "-t requires an argument\n");
                    return 1;
                }
                id = argv[1];
                --argc;
            ...
            if (*id != '\0') {
                ...
                capture_printf(out_buf, out_buf_size, "invalid transport id\n");
                return 1;
            }
            ...
            if (argv[0][2] == '\0') {
                if (argc < 2) {
                    // error_exit("-H requires an argument");
                    capture_printf(out_buf, out_buf_size, "-H requires an argument\n");
                    return 1;
                }
            ...
            if (argv[0][2] == '\0') {
                if (argc < 2) {
                    // error_exit("-P requires an argument");
                    capture_printf(out_buf, out_buf_size, "-P requires an argument\n");
                    return 1;
                }
            ...
            if (argc < 2) {
                // error_exit("-L requires an argument");
                capture_printf(out_buf, out_buf_size, "-L requires an argument\n");
                return 1;
            }
    ...
    if ((server_host_str || server_port_str) && server_socket_str) {
        ...
        capture_printf(out_buf, out_buf_size, "-L is incompatible with -H or -P\n");
        return 1;
    }
    ...
    if (!server_socket_str) {
        ...
        if (server_port_str && strlen(server_port_str) > 0) {
            if (!android::base::ParseInt(server_port_str, &server_port, 1, 65535)) {
                ...
                //         "got \"%s\"",
                //         server_port_str);
                capture_printf(out_buf, out_buf_size,
                    "$ANDROID_ADB_SERVER_PORT must be a positive number less than 65535: "
                    "got \"%s\"\n",
                    server_port_str);
                return 1;
    ...
    if (one_device_str && !server_start) {
        ...
        capture_printf(out_buf, out_buf_size, "--one-device is only allowed when starting a server.\n");
        return 1;
    }
    ...
    if (is_server) {
        if (no_daemon || is_daemon) {
            if (is_daemon && (ack_reply_fd == -1)) {
                ...
                capture_printf(out_buf, out_buf_size, "reply fd for adb server to client communication not specified.\n");
                return 1;
            }
        ...
        if (r) {
            ...
            capture_printf(out_buf, out_buf_size, "* could not start server *\n");
        }
        return r;
    }
    ...
    if (!strcmp(argv[0], "devices")) {
        ...
        if (argc < 2) {
            ...
            capture_printf(out_buf, out_buf_size, "adb devices [-l]\n");
            return 1;
        }
        ...
        if (!adb_check_server_version(&error)) {
            ...
            capture_printf(out_buf, out_buf_size, "failed to check server version: %s\n", error.c_str());
            return 1;
        }
        printf("List of devices attached\n");
        capture_printf(out_buf, out_buf_size, "List of devices attached\n");
        return adb_query_command(out_buf, out_buf_size, query);
        ...
        printf("%" PRIu64 "\n", transport_id);
        capture_printf(out_buf, out_buf_size, "%" PRIu64 "\n", transport_id);
        return 0;
    } else if (!strcmp(argv[0], "connect")) {
        if (argc != 2) {
            // error_exit("usage: adb connect HOST[:PORT]");
            capture_printf(out_buf, out_buf_size, "usage: adb connect HOST[:PORT]\n");
            return 1;
        }
        ...
        if (argc > 2) {
            // error_exit("usage: adb disconnect [HOST[:PORT]]");
            capture_printf(out_buf, out_buf_size, "usage: adb disconnect [HOST[:PORT]]\n");
            return 1;
        }
        ...
        if (argc < 2 || argc > 3) {
            error_exit("usage: adb pair HOST[:PORT] [PAIRING CODE]");
            capture_printf(out_buf, out_buf_size, "usage: adb pair HOST[:PORT] [PAIRING CODE]\n");
            return 1;
        }
        ...
        if (argc == 2) {
            ...
            capture_printf(out_buf, out_buf_size, "Enter pairing code: ");
            fflush(stdout);
            if (!std::getline(std::cin, password) || password.empty()) {
                // error_exit("No pairing code provided");
                capture_printf(out_buf, out_buf_size, "No pairing code provided\n");
                return 1;
            }
        } else {
            password = argv[2];
        ...
        return adb_shell(out_buf, out_buf_size, argc, argv);
    } else if (!strcmp(argv[0], "exec-in") || !strcmp(argv[0], "exec-out")) {
        int exec_in = !strcmp(argv[0], "exec-in");

        if (argc < 2) {
            // error_exit("usage: adb %s command", argv[0]);
            capture_printf(out_buf, out_buf_size, "usage: adb %s command\n", argv[0]);
            return 1;
        }
        ...
        if (fd < 0) {
            fprintf(stderr, "error: %s\n", error.c_str());
            capture_printf(out_buf, out_buf_size, "error: %s\n", error.c_str());
            return -1;
        }
        ...
        if (argc != 2) {
            // error_exit("sideload requires an argument");
            capture_printf(out_buf, out_buf_size, "sideload requires an argument\n");
            return 1;
        }
        if (adb_sideload_install(out_buf, out_buf_size, argv[1], false /* rescue_mode */)) {
            return 1;
        } else {
            return 0;
        }
        ...
        if (argc < 2) {
            // error_exit("rescue requires at least one argument");
            capture_printf(out_buf, out_buf_size, "rescue requires at least one argument\n");
            return 1;
        }
        if (!strcmp(argv[1], "getprop")) {
            ...
            capture_printf(out_buf, out_buf_size, "invalid rescue getprop arguments\n");
            return 1;
        } else if (!strcmp(argv[1], "install")) {
            if (argc != 3) {
                // error_exit("rescue install requires two arguments");
                capture_printf(out_buf, out_buf_size, "rescue install requires two arguments\n");
                return 1;
            }
            if (adb_sideload_install(out_buf, out_buf_size, argv[2], true /* rescue_mode */) != 0) {
                return 1;
            }
        } else if (!strcmp(argv[1], "wipe")) {
            if (argc != 3 || strcmp(argv[2], "userdata") != 0) {
                // error_exit("invalid rescue wipe arguments");
                capture_printf(out_buf, out_buf_size, "invalid rescue wipe arguments\n");
                return 1;
            }
            return adb_wipe_devices();
        } else {
            // error_exit("invalid rescue argument");
            capture_printf(out_buf, out_buf_size, "invalid rescue argument\n");
            return 1;
        }
        return 0;
    } else if (!strcmp(argv[0], "tcpip")) {
        if (argc != 2) {
            // error_exit("tcpip requires an argument");
            capture_printf(out_buf, out_buf_size, "tcpip requires an argument\n");
            return 1;
        }
        int port;
        if (!android::base::ParseInt(argv[1], &port, 1, 65535)) {
            // error_exit("tcpip: invalid port: %s", argv[1]);
            capture_printf(out_buf, out_buf_size, "tcpip: invalid port: %s\n", argv[1]);
            return 1;
        }
        ...
        --argc;
        if (argc < 1) {
            // error_exit("%s requires an argument", argv[0]);
            capture_printf(out_buf, out_buf_size, "%s requires an argument\n", argv[0]);
            return 1;
        }
        ++argv;
        ...
        if (strcmp(argv[0], "--list") == 0) {
            if (argc != 1) {
                // error_exit("--list doesn't take any arguments");
                capture_printf(out_buf, out_buf_size, "--list doesn't take any arguments\n");
                return 1;
            }
            return adb_query_command(out_buf, out_buf_size, host_prefix + "list-forward");
        } else if (strcmp(argv[0], "--remove-all") == 0) {
            if (argc != 1) {
                // error_exit("--remove-all doesn't take any arguments");
                capture_printf(out_buf, out_buf_size, "--remove-all doesn't take any arguments\n");
                return 1;
            }
            cmd = "killforward-all";
            ...
            if (argc != 2) {
                // error_exit("--remove requires an argument");
                capture_printf(out_buf, out_buf_size, "--remove requires an argument\n");
                return 1;
            }
            ...
            // forward --no-rebind <local> <remote>
            if (argc != 3) {
                // error_exit("--no-rebind takes two arguments");
                capture_printf(out_buf, out_buf_size, "--no-rebind takes two arguments\n");
                return 1;
            }
            ...
            if (argc != 2) {
                // error_exit("forward takes two arguments");
                capture_printf(out_buf, out_buf_size, "forward takes two arguments\n");
                return 1;
            }
        ...
        --argc;
        if (argc < 1) {
            // error_exit("mdns requires an argument");
            capture_printf(out_buf, out_buf_size, "mdns requires an argument\n");
            return 1;
        }
        ++argv;
        ...
        if (!strcmp(argv[0], "check")) {
            if (argc != 1) {
                ...
                capture_printf(out_buf, out_buf_size, "mdns %s doesn't take any arguments\n", argv[0]);
                return 1;
            }
            query += "check";
            ...
            capture_printf(out_buf, out_buf_size, "List of discovered mdns services\n");
        } else {
            // error_exit("unknown mdns command [%s]", argv[0]);
            capture_printf(out_buf, out_buf_size, "unknown mdns command [%s]\n", argv[0]);
            return 1;
        }
    ...
    /* do_sync_*() commands */
        ...
        if (argc != 2) {
            // error_exit("ls requires an argument");
            capture_printf(out_buf, out_buf_size, "ls requires an argument\n");
            return 1;
        }
        return do_sync_ls(out_buf, out_buf_size, argv[1]) ? 0 : 1;
        ...
        if (srcs.empty() || !dst) {
            ...
            capture_printf(out_buf, out_buf_size, "push requires <source> and <destination> arguments\n");
            return 1;
        }

        return do_sync_push(out_buf, out_buf_size, srcs, dst, sync, compression, dry_run, quiet) ? 0 : 1;
        ...
        parse_push_pull_args(&argv[1], argc - 1, &srcs, &dst, &copy_attrs, nullptr, &quiet,
                             &compression, nullptr);
        if (srcs.empty()) {
            // error_exit("pull requires an argument");
            capture_printf(out_buf, out_buf_size, "pull requires an argument\n");
            return 1;
        }
        return do_sync_pull(out_buf, out_buf_size, srcs, dst, copy_attrs, compression, nullptr, quiet) ? 0 : 1;
    } else if (!strcmp(argv[0], "install")) {
        if (argc < 2) {
            // error_exit("install requires an argument");
            capture_printf(out_buf, out_buf_size, "install requires an argument\n");
            return 1;
        }
        return install_app(argc, argv);
    } else if (!strcmp(argv[0], "install-multiple")) {
        if (argc < 2) {
            // error_exit("install-multiple requires an argument");
            capture_printf(out_buf, out_buf_size, "install-multiple requires an argument\n");
            return 1;
        }
        return install_multiple_app(argc, argv);
    } else if (!strcmp(argv[0], "install-multi-package")) {
        if (argc < 2) {
            // error_exit("install-multi-package requires an argument");
            capture_printf(out_buf, out_buf_size, "install-multi-package requires an argument\n");
            return 1;
        }
        return install_multi_package(argc, argv);
    } else if (!strcmp(argv[0], "uninstall")) {
        if (argc < 2) {
            // error_exit("uninstall requires an argument");
            capture_printf(out_buf, out_buf_size, "uninstall requires an argument\n");
            return 1;
        }
        ...
        while ((opt = getopt(argc, const_cast<char**>(argv), "lnz:Zq")) != -1) {
            switch (opt) {
                ...
                default:
                    ...
                    capture_printf(out_buf, out_buf_size, "usage: adb sync [-l] [-n]  [-z ALGORITHM] [-Z] [-q] [PARTITION]\n");
                    return 1;
            }
        }
        ...
        for (const auto& partition : partitions) {
            if (src == "all" || src == partition) {
                ...
                found = true;
                if (!do_sync_sync(out_buf, out_buf_size, src_dir, "/" + partition, list_only, compression, dry_run, quiet)) {
                    return 1;
                }
            }
        }
        if (!found) {
            // error_exit("don't know how to sync %s partition", src.c_str());
            capture_printf(out_buf, out_buf_size, "don't know how to sync %s partition\n", src.c_str());
            return 1;
        }
        return 0;
    }
    /* passthrough commands */
        ...
        return adb_query_command(out_buf, out_buf_size, format_host_command(argv[0]));
    }
    /* other commands */
        ...
        if (argc != 2) {
            // error_exit("keygen requires an argument");
            capture_printf(out_buf, out_buf_size, "keygen requires an argument\n");
            return 1;
        }
        ...
        if (argc != 2) {
            // error_exit("pubkey requires an argument");
            capture_printf(out_buf, out_buf_size, "pubkey requires an argument\n");
            return 1;
        }
        ...
        if (argc == 1) {
            ...
            capture_printf(out_buf, out_buf_size, "usage: adb track-app [--proto-binary][--proto-text]\n");
            return 1;
        }
        ...
        if (argc < 2) {
            ...
            if (!strcmp(argv[1], "-l")) {
                ...
                capture_printf(out_buf, out_buf_size, "usage: adb track-devices [-l][--proto-text][--proto-binary]\n");
                return 1;
            }
        }
        ...
        if (argc != 2) {
            ...
            capture_printf(out_buf, out_buf_size, "usage: adb raw SERVICE\n");
            return 1;
        }
    ...
    /* "adb /?" is a common idiom under Windows */
        ...
        capture_printf(out_buf, out_buf_size, "%s", adb_version().c_str());
        return 0;
        ...
        return adb_query_command(out_buf, out_buf_size, "host:host-features");
        ...
        if (argc == 1) {
            ...
            if (!strcmp(argv[1], "device")) {
                ...
                std::string err;
                return adb_query_command(out_buf, out_buf_size, "host:reconnect-offline");
            } else {
                // error_exit("usage: adb reconnect [device|offline]");
                capture_printf(out_buf, out_buf_size, "usage: adb reconnect [device|offline]\n");
                return 1;
            }
        }
        ...
        if (argc < 4) {
            ...
            capture_printf(out_buf, out_buf_size, "usage: adb inc-server CONNECTION_HANDLE OUTPUT_HANDLE FILE1 FILE2 ...");
            return 1;
#else
            error_exit("usage: adb inc-server CONNECTION_FD OUTPUT_FD FILE1 FILE2 ...");
            capture_printf(out_buf, out_buf_size, "usage: adb inc-server CONNECTION_FD OUTPUT_FD FILE1 FILE2 ...");
            return 1;
#endif
        }
        ...
        if (!_is_valid_os_fd(connection_fd)) {
            ...
            capture_printf(out_buf, out_buf_size, "Invalid connection_fd number given: %d\n", connection_fd);
            return 1;
        }
        ...
        if (!_is_valid_os_fd(output_fd)) {
            ...
            capture_printf(out_buf, out_buf_size, "Invalid output_fd number given: %d\n", output_fd);
            return 1;
        }
        ...
        if (!adb_query(service, &result, &error, true)) {
            ...
            capture_printf(out_buf, out_buf_size, "failed to %s: %s\n", argv[0], error.c_str());
            return 1;
        }
        printf("%s\n", result.c_str());
        capture_printf(out_buf, out_buf_size, "%s\n", result.c_str());
    ...
    capture_printf(out_buf, out_buf_size, "unknown command %s\n", argv[0]);
    return 1;
