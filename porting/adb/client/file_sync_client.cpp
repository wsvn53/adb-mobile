...
struct TransferLedger {
  ...
  public:
    ...
    void ReportTransferRate(char **output_buffer, size_t *output_size, LinePrinter& lp, const std::string& name, TransferDirection direction) {
        ...
        capture_printf(output_buffer, output_size, ss.str().c_str());
...
class SyncConnection {
  public:
    SyncConnection(char **output_buffer, size_t *output_size) : acknowledgement_buffer_(sizeof(sync_status) + SYNC_DATA_MAX) {
        ...
        if (!features) {
            capture_printf(output_buffer, output_size, "failed to get feature set: %s", error.c_str());
            ...
            if (fd < 0) {
                capture_printf(output_buffer, output_size, "connect failed: %s", error.c_str());
    ...
    void ReportTransferRate(char **output_buffer, size_t *output_size, const std::string& file, TransferDirection direction) {
        current_ledger_.ReportTransferRate(output_buffer, output_size, line_printer_, file, direction);
    }

    void ReportOverallTransferRate(TransferDirection direction) {
        if (current_ledger_ != global_ledger_) {
            global_ledger_.ReportTransferRate(nullptr, nullptr, line_printer_, "", direction);
        }
    }
...
bool do_sync_ls(char **output_buffer, size_t *output_size, const char* path) {
    SyncConnection sc(output_buffer, output_size);
    if (!sc.IsValid()) return false;

    return sync_ls(sc, path, [output_buffer, output_size](unsigned mode, uint64_t size, uint64_t time, const char* name) {
        capture_printf(output_buffer, output_size, "%08x %08" PRIx64 " %08" PRIx64 " %s\n", mode, size, time, name);
...
static bool copy_local_dir_remote(char **output_buffer, size_t *output_size, SyncConnection& sc, std::string lpath, std::string rpath,
    ...
    if (!CanUseFeature(sc.Features(), kFeatureFixedPushMkdir) &&
        ...
        for (const auto& dir : directory_list) {
            ...
            if (escaped_path.size() > 16384) {
                ...
                capture_printf(output_buffer, output_size, "path too long: %s\n", escaped_path.c_str());
    ...
    if (check_timestamps) {
        for (const copyinfo& ci : file_list) {
            if (!sc.SendLstat(ci.rpath)) {
                capture_printf(output_buffer, output_size, "failed to send lstat\n");
    ...
    for (const copyinfo& ci : file_list) {
        if (!ci.skip) {
            if (list_only) {
                capture_printf(output_buffer, output_size, "would push: %s -> %s", ci.lpath.c_str(), ci.rpath.c_str());
    ...
    sc.ReportTransferRate(nullptr, nullptr, lpath, TransferDirection::push);
    return success;
}

bool do_sync_push(char **output_buffer, size_t *output_size, const std::vector<const char*>& srcs, const char* dst, bool sync,
    ...
    if (sync_stat_fallback(sc, dst, &st)) {
        ...
        if (errno == ENOENT || errno == ENOPROTOOPT) {
            ...
            capture_printf(output_buffer, output_size, "stat failed when trying to push to %s: %s", dst, strerror(errno));
    ...
    if (!dst_isdir) {
        if (srcs.size() > 1) {
            capture_printf(output_buffer, output_size, "target '%s' is not a directory", dst);
            ...
            if (dst[dst_len - 1] == '/' && dst_exists) {
                capture_printf(output_buffer, output_size, "failed to access '%s': Not a directory", dst);
    ...
    for (const char* src_path : srcs) {
        ...
        if (stat(src_path, &st) == -1) {
            capture_printf(output_buffer, output_size, "cannot stat '%s': %s", src_path, strerror(errno));
        ...
        if (S_ISDIR(st.st_mode)) {
            ...
            success &=
                    copy_local_dir_remote(output_buffer, output_size, sc, src_path, dst_dir, sync, false, compression, dry_run);
            continue;
        ...
        success &= sync_send(sc, src_path, dst_path, st.st_mtime, st.st_mode, sync, compression,
                             dry_run);
        sc.ReportTransferRate(output_buffer, output_size, src_path, TransferDirection::push);
    }
...
static bool copy_remote_dir_local(char **output_buffer, size_t *output_size, SyncConnection& sc, std::string rpath, std::string lpath,
    ...
    // Recursively build the list of files to copy.
    capture_printf(output_buffer, output_size, "pull: building file list...");
    ...
    for (const copyinfo &ci : file_list) {
        if (!ci.skip) {
            if (S_ISDIR(ci.mode)) {
                ...
                // TODO(b/25457350): We don't preserve permissions on directories.
                if (!mkdirs(ci.lpath))  {
                    capture_printf(output_buffer, output_size, "failed to create directory '%s': %s",
    ...
    sc.RecordFilesSkipped(skipped);
    sc.ReportTransferRate(output_buffer, output_size, rpath, TransferDirection::pull);
    return true;
}

bool do_sync_pull(char **output_buffer, size_t *output_size, const std::vector<const char*>& srcs, const char* dst, bool copy_attrs,
    ...
    if (stat(dst, &st) == -1) {
        ...
        if (srcs.size() == 1 && errno == ENOENT) {
            ...
            if (stat(android::base::Dirname(dst).c_str(), &parent_st) == -1) {
                capture_printf(output_buffer, output_size, "cannot create file/directory '%s': %s", dst, strerror(errno));
                sc.Error("cannot create file/directory '%s': %s", dst, strerror(errno));
                return false;
            }
        } else {
            capture_printf(output_buffer, output_size, "failed to access '%s': %s", dst, strerror(errno));
    ...
    for (const char* src_path : srcs) {
        ...
        if (!sync_stat_fallback(sc, src_path, &src_st)) {
            if (errno == ENOPROTOOPT) {
                capture_printf(output_buffer, output_size, "remote object '%s' does not exist\n", src_path);
                sc.Error("remote object '%s' does not exist", src_path);
            } else {
                capture_printf(output_buffer, output_size, "failed to stat remote object '%s': %s\n", src_path, strerror(errno));
        ...
        if (src_isdir) {
            ...
            if (dst_exists) {
                if (!dst_isdir) {
                    capture_printf(output_buffer, output_size, "target '%s' is not a directory\n", dst);
                    sc.Error("target '%s' is not a directory", dst);
                    return false;
                }
            ...
            success &= copy_remote_dir_local(output_buffer, output_size, sc, src_path, dst_dir, copy_attrs, compression);
            continue;
        ...
        sc.ReportTransferRate(output_buffer, output_size, src_path, TransferDirection::pull);
    }
...
bool do_sync_sync(char **output_buffer, size_t *output_size, const std::string& lpath, const std::string& rpath, bool list_only,
                  CompressionType compression, bool dry_run, bool quiet) {
    SyncConnection sc(output_buffer, output_size);
    if (!sc.IsValid()) return false;
    sc.SetQuiet(quiet);

    bool success = copy_local_dir_remote(output_buffer, output_size, sc, lpath, rpath, true, list_only, compression, dry_run);
    if (!list_only) {
        sc.ReportOverallTransferRate(TransferDirection::push);
    }
    return success;
